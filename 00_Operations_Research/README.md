# Modül 0: Yöneylem Araştırması ve Lineer Programlama 🎯

> "Maksimum çıktı, minimum girdi; kısıtlar altında en iyi çözüm."

Yöneylem Araştırması (Operations Research), karmaşık karar verme problemlerinin matematiksel modellerle çözülmesidir. Bu modül, Endüstri Mühendisliğinin kalbi sayılan optimizasyon tekniklerine odaklanır.
...
$$\text{Kısıtlar:} \quad \sum_{j=1}^{n} a_{ij} x_j \leq b_i \quad (i=1, \dots, m)$$
$$x_j \geq 0$$
...
- **Ekonomik Yorum:** Dual değişkenler, kısıtların "Gölge Fiyatlarını" (Shadow Prices) temsil eder.

---

## 🛠️ Dahili Araçlar

### [simplex_solver_utils.py](simplex_solver_utils.py)
`SciPy` kütüphanesini kullanarak, karmaşık üretim karması (product mix) ve kaynak atama problemlerini çözen profesyonel bir LP çözücüdür.
